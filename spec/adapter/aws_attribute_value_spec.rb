    ...
    describe AttributeValue do
      ...
      describe '#marshal' do
        ...
        it 'converts hashes to :m (map)' do
          formatted = value.marshal(type: Hash, value: {foo: 'bar', abc: 'mno'})
        ...
        it 'converts arrays to :l (list)' do
          formatted = value.marshal(type: Array, value: ['abc', 'mno'])
        ...
        it 'converts IO objects to :b (binary)' do
          io = StringIO.new('bar')
          formatted = value.marshal(type: Hash, value: {foo: io})
          expect(formatted[:m]["foo"][:b].read).to eq('bar')
        end

        it 'converts string sets to :ss (string set)' do
          formatted = value.marshal(type: Set, value: Set.new(%w(abc mno)))
          expect(formatted).to eq(ss: %w(abc mno))
        end

        it 'converts symbol sets to :ss (string set)' do
          formatted = value.marshal(type: Set, value: Set.new([:abc, :mno]))
          expect(formatted).to eq(ss: %w(abc mno))
        end

        it 'converts numeric sets to :ns (number set)' do
          formatted = value.marshal(type: Set, value: Set.new([123, 456]))
          expect(formatted).to eq(ns: %w(123 456))
        end

        it 'converts binary sets to :bs (binary set)' do
          io_obj = StringIO.new('data')
          formatted = value.marshal(type: 'auto', value: Set.new([io_obj]))
          expect(formatted).to eq(bs: [io_obj])
        end

        it 'converts numerics to :n (number)' do
          # supports integers, floats, and big decimals
          expect(value.marshal(type: Numeric, value: 123)).to eq(n: '123')
          expect(value.marshal(type: Numeric, value: 12.34)).to eq(n: '12.34')
          expect(value.marshal(type: Numeric, value: BigDecimal.new("0.1E125"))).to eq(n: "0.1E125")
        end

        it 'converts strings to :s' do
          expect(value.marshal(type: String, value: 'abc')).to eq(s: 'abc')
        end

        it 'converts symbol to :s' do
          expect(value.marshal(type: Symbol, value: :abc)).to eq(s: 'abc')
        end

        it 'converts booleans :bool' do
          # supports both true and false
          expect(value.marshal(type: TrueClass, value: true)).to eq(bool: true)
          expect(value.marshal(type: FalseClass, value: false)).to eq(bool: false)
        end

        it 'converts nil to :null' do
          expect(value.marshal(type: NilClass, value: nil)).to eq(null: true)
        end

        it 'recursively converted mixed types' do
          formatted = value.marshal(type: Hash, value: {
        ...
        it 'raises an argument error for unhandled types' do
          expect {
            value.marshal(type: Time, value: Time.now)
